...
}, xhttp.open("GET", "assets/json/support-tickets-list.json"), xhttp.send(), isCount = (new DOMParser).parseFromString(ticketsList.items.slice(-1)[0]._values.id, "text/html");
...
refreshCallbacks(), document.getElementById("showModal").addEventListener("show.bs.modal", function(e) {
  e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Ticket", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").style.display = "none", document.getElementById("edit-btn").style.display = "block") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("modal-id").style.display = "none", document.getElementById("exampleModalLabel").innerHTML = "Add Ticket", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("edit-btn").style.display = "none", document.getElementById("add-btn").style.display = "block") : (document.getElementById("exampleModalLabel").innerHTML = "List Ticket", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none")
...
addBtn.addEventListener("click", function(e) {
  "" !== tasksTitleField.value && "" !== client_nameNameField.value && "" !== assignedtoNameField.value && "" !== dateField.value && "" !== dateDueField.value && "" !== statusField.value && "" !== priorityField.value && (ticketsList.add({
  ...
  }), document.getElementById("close-modal").click(), clearFields(), refreshCallbacks(), filterOrder("All"), count++)
}), editBtn.addEventListener("click", function(e) {
  document.getElementById("exampleModalLabel").innerHTML = "Edit Order", ticketsList.get({
    id: idField.value
  }).forEach(function(e) {
...
function refreshCallbacks() {
  removeBtns.forEach(function(e) {
    e.addEventListener("click", function(e) {
      e.target.closest("tr").children[1].innerText, itemId = e.target.closest("tr").children[1].innerText, ticketsList.get({
        id: itemId
      }).forEach(function(e) {
    ...
    e.addEventListener("click", function(e) {
      e.target.closest("tr").children[1].innerText, itemId = e.target.closest("tr").children[1].innerText, ticketsList.get({
        ...
        t == itemId && (idField.value = t, tasksTitleField.value = e._values.tasks_name, client_nameNameField.value = e._values.client_name, assignedtoNameField.value = e._values.assignedto, dateField.value = e._values.create_date, dateDueField.value = e._values.due_date, example && example.destroy(), example = new Choices(priorityField, {
          searchEnabled: !1
        }), val = (new DOMParser).parseFromString(e._values.priority, "text/html"), t = val.body.firstElementChild.innerHTML, example.setChoiceByValue(t), statusVal && statusVal.destroy(), statusVal = new Choices(statusField, {
          searchEnabled: !1
...
function deleteMultiple() {
  ids_array = [];
  for (var e, t = document.querySelectorAll(".form-check [value=option1]"), a = 0; a < t.length; a++) 1 == t[a].checked && (e = t[a].parentNode.parentNode.parentNode.querySelector("td [data-id]").getAttribute("data-id"), ids_array.push(e));
